...
class KO7_ORM extends Model implements Serializable {
	...
	public function get($column)
	{
		...
		{
			throw new KO7_Exception('The :property property does not exist in the :class class',
	...
	public function set($column, $value)
	{
		...
		elseif (isset($this->_has_many[$column]))
		{
			...
			{
				throw new KO7_Exception('The :property: property is a to many relation in the :class: class',
					[':property:' => $column, ':class:' => get_class($this)]);
			}
		}
		else
	...
	{
		if ($this->_loaded)
			throw new KO7_Exception('Cannot create :model model because it is already loaded.', [':model' => $this->_object_name]);
	...
	{
		if ( ! $this->_loaded)
			throw new KO7_Exception('Cannot update :model model because it is not loaded.', [':model' => $this->_object_name]);
	...
	{
		if ( ! $this->_loaded)
			throw new KO7_Exception('Cannot delete :model model because it is not loaded.', [':model' => $this->_object_name]);
